...
class Lift_Admin_Old {
	/**
	 * Page name used for admin setup 
	 */

	const LANDING_PAGE = 'lift-search/admin-old/setup.php';

	/**
	 * Page name used for admin status 
	 */
	const STATUS_PAGE = 'lift-search/admin-old/status.php';
	...
	public static function settings_link( $links, $page ) {
		if ( $page == plugin_basename( __FILE__ ) ) {
			$links[] = '<a href="' . admin_url( 'options-general.php?page=' . self::LANDING_PAGE ) . '">Settings</a>';
		}
		return $links;
	}

	public static function _enqueue_style() {
		wp_enqueue_style( 'lift-search-admin', plugins_url( 'css/admin-old.css', __DIR__ ) );
	}
	...
	public static function _setup_management_pages() {
		...
		add_options_page( 'Lift: Search for WordPress', 'Lift Search Old', $capability, self::STATUS_PAGE );
		add_submenu_page( '', 'Lift: Search for Wordpress', 'Lift Search Old', $capability, self::LANDING_PAGE );
		...
		$func_enqueue_admin_script = function() {
				...
				Lift_Admin_Old::_enqueue_style();
			};

		foreach ( array( self::LANDING_PAGE, self::STATUS_PAGE ) as $hook ) {
